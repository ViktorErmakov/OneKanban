	...
	Если ДопустимоеСобытие <> Неопределено Тогда
		...
		Если ИмяСобытия = "ОбновитьКанбанДоскуHTML" Тогда
			...
			Если ЗначениеЗаполнено(Параметр) Тогда
				...
				ЗаполнитьДанныеЗадачиДляОтвета(Параметр, ДанныеЗадачи);
...
Функция ЗагрузитьТекщиеНастройкиДоски()
	...
	КлючОбъекта = КлючОбъектаНастроекДоски();
	КлючНастроек = КлючНастроекДоски();
	НастройкиДоски = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить(КлючОбъекта, КлючНастроек); // см. НовыйНастройкиДоски
...
Процедура ЗаполнитьДанныеЗадачиДляОтвета(Знач ЗадачаСсылка, ДанныеЗадачи)
	...
	ИначеЕсли Метаданные.ОпределяемыеТипы.канбан_Ошибки.Тип.СодержитТип(ТипЗнч(ЗадачаСсылка)) Тогда
