...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	канбан_СтатистикаИспользования.ЗафиксироватьОткрытиеДоски();
...
Процедура КанбанДоскаПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если Кнопка.id = "V8_request" Тогда
		...
		ИначеЕсли Кнопка.value = "openBoardCreateItem" Тогда
			
			// Создание задачи или ошибки: параметры из HTML (V8Proxy.fetch), см. OneKanban-html
			Если ИзмененнаяЗадача <> Неопределено Тогда
				
				ВидСоздания = ЗначениеПараметраСозданияСДоски(ИзмененнаяЗадача, "createKind", "createkind");
				ИдентификаторСтатусаСтрока = ЗначениеПараметраСозданияСДоски(
					ИзмененнаяЗадача, "idStatusColumn", "idstatuscolumn");
				ИмяМенеджераОбъектаСтатуса = ЗначениеПараметраСозданияСДоски(
					ИзмененнаяЗадача, "fullNameObjectStatus", "fullnameobjectstatus");
				
				Если Не ПустаяСтрока(ВидСоздания) И Не ПустаяСтрока(ИдентификаторСтатусаСтрока)
					И Не ПустаяСтрока(ИмяМенеджераОбъектаСтатуса) Тогда
					
					СтатусЗадачиСсылкаКолонки = СтатусЗадачиСсылка(ИмяМенеджераОбъектаСтатуса, ИдентификаторСтатусаСтрока);
					ОткрытьФормуСозданияЭлементаДоскиПоВиду(ВидСоздания, СтатусЗадачиСсылкаКолонки);
...
Процедура ТекстHTMLКанбанДоски()
	...
	ПоказФотоИсполнителя = канбан_КанбанДоска.ПоказыватьФотографиюИсполнителяПоНастройкамДоски();
	
	Если Параметры.Свойство("СкрыватьФотоПользователя") И Параметры.СкрыватьФотоПользователя Тогда
		ПоказФотоИсполнителя = Ложь;
	КонецЕсли;
	
	канбан_КанбанДоска.ТекстHTMLКанбанДоски(КанбанДоска, УникальныйИдентификатор, ПоказФотоИсполнителя);
...
&НаСервереБезКонтекста
Функция ПараметрыОткрытияФормыСозданияНаСервере(Знач ВидСоздания, Знач СтатусЗадачиКолонки)
	
	Возврат канбан_КанбанДоска.ПараметрыОткрытияФормыСозданияСЭлементыДоски(ВидСоздания, СтатусЗадачиКолонки);
	
КонецФункции

&НаКлиенте
Процедура ОткрытьФормуСозданияЭлементаДоскиПоВиду(Знач ВидСоздания, Знач СтатусЗадачиКолонки)
	
	ПараметрыФормы = ПараметрыОткрытияФормыСозданияНаСервере(ВидСоздания, СтатусЗадачиКолонки);
	ОткрытьФорму(ПараметрыФормы.ИмяФормы, ПараметрыФормы.ПараметрыОткрытия);
	
КонецПроцедуры

// Значение строкового параметра из структуры JSON (два варианта имени ключа).
//
// Параметры:
//  Данные - Структура, Соответствие - распознанный JSON из атрибута task
//  ИмяКлюча1 - Строка
//  ИмяКлюча2 - Строка
//
// Возвращаемое значение:
...
//
&НаКлиенте
Функция ЗначениеПараметраСозданияСДоски(Знач Данные, Знач ИмяКлюча1, Знач ИмяКлюча2)
	...
	Если Данные.Свойство(ИмяКлюча1, Значение) Тогда
		Возврат Строка(Значение);
	КонецЕсли;
	
	Если Данные.Свойство(ИмяКлюча2, Значение) Тогда
		Возврат Строка(Значение);
	КонецЕсли;
