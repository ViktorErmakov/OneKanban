...
Процедура ТекстHTMLКанбанДоски(
	...
	ДанныеДляДоски = канбан_КанбанДоска.ДанныеДляКанбанДоски(ПоказыватьФотоПользователя);
	...
	// Логотип доски
	УзелЛоготипДоски = ДокументHTML.ПолучитьЭлементПоИдентификатору("logo_img");
	ДвоичныеДанныеКартинки = БиблиотекаКартинок.канбан_ЛоготипТекст.ПолучитьДвоичныеДанные();
	КартинкаСтрокаBase64 = Base64Строка(ДвоичныеДанныеКартинки);
	УзелЛоготипДоски.УстановитьАтрибут("src", СтрШаблон("data:image/png;base64,%1", КартинкаСтрокаBase64));
	
	//	Начальные настройки доски
	УзелНастройкиДоски = ДокументHTML.ПолучитьЭлементПоИдентификатору("board-settings");
	НастройкиДоски = канбан_КанбанДоска.ЗагрузитьТекщиеНастройкиДоски();
	УзелНастройкиДоски.ТекстовоеСодержимое = ЗаписатьЗначениеJSON(НастройкиДоски);
	
	УзелДанныеПроектов = ДокументHTML.ПолучитьЭлементПоИдентификатору("projects-data");
	...
	Для Каждого ДанныеПроекта Из ПроектыДоски Цикл
		...
		СтруктураПроекта.Вставить("id", ИД_Проекта);
		СтруктураПроекта.Вставить("name", Строка(ДанныеПроекта.ПредставлениеПроекта));
		СтруктураПроекта.Вставить("color", "");
		
		Если ЗначениеЗаполнено(ДанныеПроекта.Цвет) Тогда
			ЦветСтрокойСЦифрами = канбан_КанбанДоска.СтрокаИзЦвета(ДанныеПроекта.Цвет);
			СтруктураПроекта.color = СтрШаблон("rgb(%1)", ЦветСтрокойСЦифрами);
		КонецЕсли;
		
		МассивПроектов.Добавить(СтруктураПроекта);
		
	КонецЦикла;
	УзелДанныеПроектов.ТекстовоеСодержимое = ЗаписатьЗначениеJSON(МассивПроектов);
...
//			УзелСсылкаЗадача.УстановитьАтрибут("href", "#");
			УзелСсылкаЗадача.УстановитьАтрибут("id", СтрШаблон("add_task_%1", УникальныйИдентификаторСтатуса));
			...
			УзелСсылкаЗадача.УстановитьАтрибут("data-full-name-object-status", ДанныеСтатуса.СтатусЗадачи.Метаданные().ПолноеИмя());
			УзелСсылкаЗадача.УстановитьАтрибут("data-status-id", УникальныйИдентификаторСтатуса);
		...
		Для Каждого СтрокаЗадачи Из ЗадачиСоСтатусом Цикл
			...
			УзелКарточка.УстановитьАтрибут(
				"class", 
				СтрШаблон("card %1 %2 %3 card__inactive", ИД_Проекта, ИД_Пользователя, Имя_Пользователя));
			...
			УзелКарточка.УстановитьАтрибут("urgencyId", СтрокаЗадачи.СрочностьИсправления);
			УзелКарточка.УстановитьАтрибут("isBug", Строка(СтрокаЗадачи.ЭтоОшибка));
			...
			УзелШапкаКарточкиСсылка.УстановитьАтрибут("draggable", "false");
...
Процедура ЗаполнитьДанныеЗадачиДляОтвета(Знач ЗадачаСсылка, ДанныеЗадачи) Экспорт
	...
	Если РезультатЗапроса.Следующий() Тогда
		...
		Если ПоказФотоИсполнителя Тогда
		...
		Если Метаданные.ОпределяемыеТипы.канбан_Ошибки.Тип.СодержитТип(ТипЗнч(ЗадачаСсылка)) Тогда
			...
			ДанныеЗадачи.status = СтатусЗадачиСтрока(СтатусЗадачи);
			
		Иначе
			
			ДанныеЗадачи.status = СтатусЗадачиСтрока(РезультатЗапроса.СтатусЗадачи);
		...
		Если ЗначениеЗаполнено(РезультатЗапроса.СрочностьИсправления) Тогда
			
			Если ОбщегоНазначения.ЭтоСправочник(РезультатЗапроса.СрочностьИсправления.Метаданные()) Тогда
				ДанныеЗадачи.urgencyId = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					РезультатЗапроса.СрочностьИсправления , "Наименование");
			ИначеЕсли ОбщегоНазначения.ЭтоПеречисление(РезультатЗапроса.СрочностьИсправления.Метаданные()) Тогда 
				ДанныеЗадачи.urgencyId = ОбщегоНазначения.ИмяЗначенияПеречисления(РезультатЗапроса.СрочностьИсправления);
			КонецЕсли;
			
		Иначе
			ДанныеЗадачи.urgencyId = "";
		КонецЕсли;
		
		ДанныеЗадачи.isBug = РезультатЗапроса.ЭтоОшибка;
...
// Статус задачи строка.
// 
// Параметры:
//  СтатусСсылка - ОпределяемыйТип.канбан_СтатусыЗадач, ОпределяемыйТип.канбан_СтатусыОшибок - Статус задачи
// 
// Возвращаемое значение:
//  Строка - Статус задачи строка
//
Функция СтатусЗадачиСтрока(СтатусСсылка)
	
	Результат = "";
	
	Если ОбщегоНазначения.ЭтоСправочник(СтатусСсылка.Метаданные()) Тогда
		Результат = Строка(СтатусСсылка.УникальныйИдентификатор());
	Иначе
		Результат = СтрЗаменить(СтатусСсылка, " ", "");
	КонецЕсли;
