...
Процедура ТекстHTMLКанбанДоски(
		ТекстКанбанДоска, ТемнаяТема, УникальныйИдентификатор, ПоказыватьФотоПользователя = Истина) Экспорт
	...
	Иначе
		...
		// Восстанавливаем фильтр по проектам
		УзелТолькоМоиЗадачи = ДокументHTML.ПолучитьЭлементПоИдентификатору("my_tasks");
	...
	// Восстанавливаем темную тему доски
	Если ТемнаяТема Тогда
		
		УзелТемнаяТема = ДокументHTML.ПолучитьЭлементПоИдентификатору("theme_toggle");
		УзелТемнаяТема.УстановитьАтрибут("class", "theme_toggle active");
		
		УзелВрапер = ДокументHTML.ПолучитьЭлементПоИдентификатору("wrapper");
		УзелВрапер.УстановитьАтрибут("class", "wrapper dark-theme");
...
//			УзелСсылкаЗадача.УстановитьАтрибут("href", "#");
		...
		Для Каждого СтрокаЗадачи Из ЗадачиСоСтатусом Цикл
			...
			ИД_Пользователя = 
				?(ПустаяСтрока(СтрокаЗадачи.ИдентификаторПользователя), "", СтрШаблон("user%1", СтрокаЗадачи.ИдентификаторПользователя));
			
			// Карточки
			УзелКарточка = ДокументHTML.СоздатьЭлемент("div");
			
			Если ТолькоМоиЗадачи(ТолькоМоиЗадачи, СтрокаЗадачи.ИдентификаторПользователя, ИдентификаторПользователя)
...
//			УзелКарточка.УстановитьАтрибут("onmouseout", "this.style.borderColor='#ebebeb';");
			...
			УзелШапкаКарточкиСсылка.ТекстовоеСодержимое = СтрШаблон("%1 №%2", СтрокаЗадачи.Задача.Метаданные().ПредставлениеОбъекта, СтрокаЗадачи.Код);
