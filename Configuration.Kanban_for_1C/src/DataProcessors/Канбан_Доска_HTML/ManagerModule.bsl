...
Процедура ТекстHTMLКанбанДоски(
	...
	УзелНастройкиДоски.ТекстовоеСодержимое = ОбщегоНазначения.ЗначениеВJSON(НастройкиДоски);
	...
	КонецЦикла;
	УзелДанныеПроектов.ТекстовоеСодержимое = ОбщегоНазначения.ЗначениеВJSON(МассивПроектов);
...
Процедура ЗаполнитьДанныеЗадачиДляОтвета(Знач ЗадачаСсылка, ДанныеЗадачи) Экспорт
	...
	Если РезультатЗапроса.Следующий() Тогда
		...
		Иначе
			...
			Если ОбщегоНазначения.ЭтоСправочник(РезультатЗапроса.СтатусЗадачи.Метаданные()) Тогда
				ДанныеЗадачи.status = Строка(РезультатЗапроса.СтатусЗадачи.УникальныйИдентификатор());
			Иначе
				ДанныеЗадачи.status = СтрЗаменить(РезультатЗапроса.СтатусЗадачи, " ", "");
			КонецЕсли;
