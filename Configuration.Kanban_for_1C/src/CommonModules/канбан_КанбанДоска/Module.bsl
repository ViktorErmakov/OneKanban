...
Процедура ТекстHTMLКанбанДоски(
		КанбанДоска, ТемнаяТема = Ложь, УникальныйИдентификатор = "", ПоказыватьФотоПользователя = Истина) Экспорт
	
	Обработки.Канбан_Доска_HTML.ТекстHTMLКанбанДоски(
		КанбанДоска, ТемнаяТема, УникальныйИдентификатор, ПоказыватьФотоПользователя);
...
//
Функция СтатусОбъектаПоИдентификатору(Знач ИмяМенеджераОбъектаЗадача, Знач ИдентификаторСтатусаСтрока) Экспорт
	
	МенеджерЗадач = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ИмяМенеджераОбъектаЗадача);
	...
	ИначеЕсли ОбщегоНазначения.ЭтоПеречисление(СтатусСсылка.Метаданные()) Тогда
		...
		СтатусСсылка = МенеджерЗадач[ИдентификаторСтатусаСтрока]; // ОпределяемыйТип.канбан_СтатусыЗадач;
		
	КонецЕсли;
	
	канбан_КанбанДоскаПереопределяемый.СтатусОбъектаПоИдентификатору(ИмяМенеджераОбъектаЗадача, ИдентификаторСтатусаСтрока, СтатусСсылка);
