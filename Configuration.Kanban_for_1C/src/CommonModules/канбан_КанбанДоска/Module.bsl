...
Процедура ТекстHTMLКанбанДоски(
		КанбанДоска, Знач УникальныйИдентификатор = "", Знач ПоказыватьФотоПользователя = Истина) Экспорт
...
//
// Параметры:
//  ИмяМенеджераОбъекта - Строка
//  ИдентификаторОбъектаСтрока	 - Строка
// 
// Возвращаемое значение:
//   - ОпределяемыйТип.канбан_СтатусыЗадач
//
Функция СсылкаНаОбъектПоИдентификатору(Знач ИмяМенеджераОбъекта, Знач ИдентификаторОбъектаСтрока) Экспорт
	
	УсловиеПроверки = Не ПустаяСтрока(ИдентификаторОбъектаСтрока) И Не ПустаяСтрока(ИмяМенеджераОбъекта);
	
	ОбщегоНазначенияКлиентСервер.Проверить(
		УсловиеПроверки, 
		НСтр("ru = 'Не все входные параметры заполнены'"), 
		"ОбщийМодуль.канбан_КанбанДоска.СтатусОбъектаПоИдентификатору");
	
	МенеджерЗадач = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ИмяМенеджераОбъекта);
	СтатусСсылка = МенеджерЗадач.ПустаяСсылка();
	
	Если ОбщегоНазначения.ЭтоСправочник(СтатусСсылка.Метаданные()) Тогда
		
		ИдентификаторСтатуса = Новый УникальныйИдентификатор(ИдентификаторОбъектаСтрока);
		СтатусСсылка = МенеджерЗадач.ПолучитьСсылку(ИдентификаторСтатуса);// ОпределяемыйТип.канбан_СтатусыЗадач, ОпределяемыйТип.канбан_СтатусыОшибок
		
	ИначеЕсли ОбщегоНазначения.ЭтоПеречисление(СтатусСсылка.Метаданные()) Тогда
		
		СтатусСсылка = МенеджерЗадач[ИдентификаторОбъектаСтрока]; // ОпределяемыйТип.канбан_СтатусыЗадач;
		
	КонецЕсли;
	
	канбан_КанбанДоскаПереопределяемый.СтатусОбъектаПоИдентификатору(
		ИмяМенеджераОбъекта, ИдентификаторОбъектаСтрока, СтатусСсылка);
...
Функция ИзменитьСтатусЗадаче(ДанныеПриИзмененииСтатуса, Отказ) Экспорт
	...
	ЗадачаСсылка = ЗадачаСсылкаПоИдентификатору(
		ДанныеПриИзмененииСтатуса.ИдентификаторЗадача, ДанныеПриИзмененииСтатуса.ИмяМенеджераОбъектаЗадача);
	
	ИсполнительСсылка = СсылкаНаОбъектПоИдентификатору(
		ПолноеИмяОбъектаМетаданныхОпределяемогоТипа("канбан_Пользователь"), 
		ДанныеПриИзмененииСтатуса.ИдентификаторИсполнителя); // ОпределяемыйТип.канбан_Пользователь
	
	СтатусСсылка = НовыйСтатусПоИдентификаторуИМенеджеруОбъекта(
		ЗадачаСсылка, ДанныеПриИзмененииСтатуса.ИдентификаторСтатуса, 
		ДанныеПриИзмененииСтатуса.ИмяМенеджераОбъектаСтатус, 
		Отказ);
	
	Если Не Отказ И Не СтатусСсылка.Пустая() Тогда
		канбан_КанбанДоскаПереопределяемый.ИзменитьСтатусЗадаче(ЗадачаСсылка, СтатусСсылка, ИсполнительСсылка);
	КонецЕсли;
...
// 
// Параметры:
//  ПоказыватьФотоИсполнителя - Булево - если Ложь, не формируются строки фото (data URI) для карточек
// 
// Возвращаемое значение:
//  см. НовыйДанныеДляКанбанДоски
//
Функция ДанныеДляКанбанДоски(Знач ПоказыватьФотоИсполнителя = Истина) Экспорт
	...
	ДанныеДляКанбанДоски.Проекты.Сортировать("ПредставлениеПроекта");
	...
	ДвоичныеДанныеФотографииПоУмолчанию = Неопределено;
	Если ПоказыватьФотоИсполнителя Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПоказыватьФотоИсполнителя Тогда
			...
			НоваяСтрока.ФотографияИсполнителя = ЗаполнитьФотографиюИсполнителя(
				Выборка.ХранилищеФотографии, ДвоичныеДанныеФотографииПоУмолчанию);
			
		Иначе
			
			НоваяСтрока.ФотографияИсполнителя = "";
		...
		НоваяСтрока.ПредставлениеПользователя = Выборка.ПредставлениеПользователя;
		
		НоваяСтрока.СрочностьИсправления = "";
		Если ЗначениеЗаполнено(Выборка.СрочностьИсправления) Тогда
			
			Если ОбщегоНазначения.ЭтоСправочник(Выборка.СрочностьИсправления.Метаданные()) Тогда
				НоваяСтрока.СрочностьИсправления = СтрЗаменить(Строка(Выборка.СрочностьИсправления), " ", "");
			ИначеЕсли ОбщегоНазначения.ЭтоПеречисление(Выборка.СрочностьИсправления.Метаданные()) Тогда
				НоваяСтрока.СрочностьИсправления = ОбщегоНазначения.ИмяЗначенияПеречисления(Выборка.СрочностьИсправления);
			КонецЕсли;
			
		КонецЕсли;
		
		НоваяСтрока.ЭтоОшибка = Выборка.ЭтоОшибка;
...
// Признаки наличия типов карточек на канбан-доске (по данным запроса доски).
//
// Возвращаемое значение:
//  Структура:
//   * ЕстьЗадачи - Булево
//   * ЕстьОшибки - Булево
//
Функция ПризнакиТиповКарточекНаДоски() Экспорт
	
	ДанныеДоски = ДанныеДляКанбанДоски();
	ЕстьЗадачи = Ложь;
	ЕстьОшибки = Ложь;
	Для Каждого СтрокаЗадачи Из ДанныеДоски.ЗадачиИСтатусы Цикл
		
		Если СтрокаЗадачи.ЭтоОшибка Тогда
			ЕстьОшибки = Истина;
		Иначе
			ЕстьЗадачи = Истина;
		КонецЕсли;
		Если ЕстьЗадачи И ЕстьОшибки Тогда
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат Новый Структура("ЕстьЗадачи, ЕстьОшибки", ЕстьЗадачи, ЕстьОшибки);
	
КонецФункции

// Разрешение вида создаваемого элемента по настройкам фильтра типа карточки и фактическому составу доски.
//
// Параметры:
...
//
// Возвращаемое значение:
//  Строка - "task", "bug" или "" (нужен выбор пользователя)
//
Функция ВидСозданияСКанбанДоскиПоНастройкам(Знач НастройкиДоски, Знач ЕстьЗадачи, Знач ЕстьОшибки) Экспорт
	
	Если Не ЕстьОшибки Тогда
		Возврат "task";
	КонецЕсли;
	Если Не ЕстьЗадачи Тогда
		Возврат "bug";
	КонецЕсли;
	
	ФильтрТипа = НастройкиДоски.cardtypefilter;
	Если ФильтрТипа.Количество() = 1 Тогда
		
		ВыбранныйТип = ФильтрТипа[0];
		Если ВыбранныйТип = "task" Тогда
			Возврат "task";
		ИначеЕсли ВыбранныйТип = "bug" Тогда
			Возврат "bug";
		КонецЕсли;
...
// Параметры открытия формы создания задачи или ошибки с колонки доски.
//
// Параметры:
//  ВидСоздания - Строка - "task" или "bug"
//  СтатусЗадачиКолонки - ОпределяемыйТип.канбан_СтатусыЗадач - статус колонки доски
//
// Возвращаемое значение:
//  Структура:
//   * ИмяФормы - Строка
...
//
Функция ПараметрыОткрытияФормыСозданияСЭлементыДоски(Знач ВидСоздания, Знач СтатусЗадачиКолонки) Экспорт
	
	УсловиеПроверки = ВидСоздания = "task" Или ВидСоздания = "bug";
	ОбщегоНазначенияКлиентСервер.Проверить(
		УсловиеПроверки,
		НСтр("ru = 'Недопустимый вид создаваемого элемента'"),
		"ОбщийМодуль.канбан_КанбанДоска.ПараметрыОткрытияФормыСозданияСЭлементыДоски");
	
	ПараметрыОткрытия = Новый Структура;
	
	Если ВидСоздания = "task" Тогда
		
		ИмяСправочника = ПолноеИмяОбъектаМетаданныхОпределяемогоТипа("канбан_Задачи");
		ПараметрыОткрытия.Вставить("Статус", СтатусЗадачиКолонки);
		
	Иначе
		
		ИмяСправочника = ПолноеИмяОбъектаМетаданныхОпределяемогоТипа("канбан_Ошибки");
		
		Если Константы.канбан_ИспользоватьСопоставлениеСтатусов.Получить() Тогда
			
			ТипСсылкиОшибки = Метаданные.ОпределяемыеТипы.канбан_СтатусыОшибок.Тип.Типы()[0];
			МетаданныеСсылкиОшибки = Метаданные.НайтиПоТипу(ТипСсылкиОшибки);
			МенеджерСтатусовОшибок = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(МетаданныеСсылкиОшибки.ПолноеИмя());
			СтатусОшибкиДляФормы = МенеджерСтатусовОшибок.ПустаяСсылка();
			
			ОтказСопоставления = Ложь;
			РегистрыСведений.канбан_СопоставлениеСтатусов.СопоставленныйСтатусОшибки(
				СтатусЗадачиКолонки, СтатусОшибкиДляФормы, ОтказСопоставления);
			Если Не ОтказСопоставления И ЗначениеЗаполнено(СтатусОшибкиДляФормы) Тогда
				ПараметрыОткрытия.Вставить("Статус", СтатусОшибкиДляФормы);
			КонецЕсли;
	...
	ИмяФормы = СтрШаблон("%1.ФормаОбъекта", ИмяСправочника);
	
	Возврат Новый Структура("ИмяФормы, ПараметрыОткрытия", ИмяФормы, ПараметрыОткрытия);
...
Процедура СопоставленныйСтатус(СтатусЗадачи, СтатусОшибки, Отказ) Экспорт
	...
	РегистрыСведений.канбан_СопоставлениеСтатусов.СопоставленныйСтатусОшибки(СтатусЗадачи, СтатусОшибки, Отказ);
	
КонецПроцедуры

// Заполнить фотографию исполнителя.
// 
// Параметры:
//  ХранилищеФотографии - ХранилищеЗначения, Строка - Хранилище фотографии
//  ДвоичныеДанныеФотографииПоУмолчанию - ДвоичныеДанные - Двоичные данные фотографии по умолчанию
// 
// Возвращаемое значение:
//  Строка - Заполнить фотографию исполнителя
//
Функция ЗаполнитьФотографиюИсполнителя(ХранилищеФотографии, ДвоичныеДанныеФотографииПоУмолчанию) Экспорт
	...
	Результат = СтрШаблон("data:image/jpeg;base64,%1", ФотографияСтрокаBase64);
...
// Используется ведение ошибок в конфигурации (для отчётов и доски).
//
// Возвращаемое значение:
//  Булево
//
Функция ИспользуетсяВедениеОшибок() Экспорт
	
	Если Метаданные.Имя = "СистемаПроектированияПрикладныхРешений" Тогда
		Возврат Истина;
	КонецЕсли;
	
	Возврат Константы.канбан_ИспользоватьСопоставлениеСтатусов.Получить();
...
Процедура СохранитьТекущиеНастройкиДоски(НастройкиДоски) Экспорт
	...
	КлючОбъекта = канбан_КанбанДоскаКлиентСервер.КлючОбъектаНастроекДоски();
	КлючНастроек = канбан_КанбанДоскаКлиентСервер.КлючНастроекДоски();
...
Функция ЗагрузитьТекщиеНастройкиДоски() Экспорт
	...
	КлючОбъекта = канбан_КанбанДоскаКлиентСервер.КлючОбъектаНастроекДоски();
	КлючНастроек = канбан_КанбанДоскаКлиентСервер.КлючНастроекДоски();
	НастройкиДоски = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить(КлючОбъекта, КлючНастроек); // см. канбан_КанбанДоскаКлиентСервер.НовыйНастройкиДоски
	
	ШаблонНастройкиДоски = канбан_КанбанДоскаКлиентСервер.НовыйНастройкиДоски();
	Если НастройкиДоски = Неопределено Тогда
		НастройкиДоски = ШаблонНастройкиДоски;
	Иначе
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(НастройкиДоски, ШаблонНастройкиДоски, Ложь);
	КонецЕсли;
	
	НастройкиДоски.urgencylevels = ПолучитьЗначенияПеречисленияПоОпределяемомуТипу("канбан_СрочностьИсправления");
	
	Возврат НастройкиДоски;
	
КонецФункции

// Признак: по сохранённым настройкам доски показывать фотографию исполнителя на карточках.
// 
// Возвращаемое значение:
//  Булево - по умолчанию Истина, если ключ в хранилище отсутствует или имеет неожиданный тип
//
Функция ПоказыватьФотографиюИсполнителяПоНастройкамДоски() Экспорт
	
	НастройкиДоскиJSON = ЗагрузитьТекщиеНастройкиДоски();
	
	Если Не НастройкиДоскиJSON.Свойство("showexecutorphoto") Тогда
		Возврат Истина;
	КонецЕсли;
	
	ЗначениеФлага = НастройкиДоскиJSON.showexecutorphoto;
	Если ТипЗнч(ЗначениеФлага) = Тип("Булево") Тогда
		Возврат ЗначениеФлага;
	ИначеЕсли ТипЗнч(ЗначениеФлага) = Тип("Строка") Тогда
		Возврат ВРег(Строка(ЗначениеФлага)) = ВРег("true") Или Строка(ЗначениеФлага) = "1";
	Иначе
		Возврат Истина;
	КонецЕсли;
...
#Область ИсторияСтатусовЗадач

// Обработчик подписки на событие канбан_ПриЗаписиЗадачи
// 
// Параметры:
//  Источник - СправочникОбъект.канбан_Задачи
//  Отказ - Булево
//
Процедура канбан_ПриЗаписиЗадачиПриЗаписи(Источник, Отказ) Экспорт
	
	Если Источник.ОбменДанными.Загрузка Тогда
		Возврат;
	КонецЕсли;
	
	ЗафиксироватьСтатус(Источник.Ссылка, Источник.Статус, Источник.Исполнитель);
	
КонецПроцедуры

// Обработчик подписки на событие канбан_ПриЗаписиОшибки
//
// Параметры:
//  Источник - СправочникОбъект.канбан_Ошибки
//  Отказ - Булево
//
Процедура канбан_ПриЗаписиОшибкиПриЗаписи(Источник, Отказ) Экспорт
...
// Обрабатывает запись объекта задачи или ошибки (форма, доска и пр.).
//
// Параметры:
//  Задача - ОпределяемыйТип.канбан_ЗадачиИОшибки
//  НовыйСтатус - ОпределяемыйТип.канбан_СтатусыЗадачИОшибок
//  Исполнитель - ОпределяемыйТип.канбан_Пользователь
//
Процедура ЗафиксироватьСтатус(Знач Задача, Знач НовыйСтатус, Знач Исполнитель) Экспорт
	
	Если Не ЗначениеЗаполнено(НовыйСтатус) Тогда
		Возврат;
	КонецЕсли;
	
	Если ЭтоОшибкаКанбан(Задача) Тогда
		
		Если Не ИспользуетсяЗаписьИсторииОшибокВРегистр() Тогда
			Возврат;
		КонецЕсли;
		
	ИначеЕсли Не ИспользуетсяЗаписьВРегистр() Тогда
		Возврат;
	КонецЕсли;
	
	Момент = ТекущаяДатаСеанса();
	
	ОткрытаяЗапись = РегистрыСведений.канбан_ИсторияСтатусовЗадач.ОткрытаяЗаписьПоЗадаче(Задача);
	
	Если ОткрытаяЗапись <> Неопределено Тогда
		
		Если ОткрытаяЗапись.Статус = НовыйСтатус Тогда
			Возврат;
		КонецЕсли;
		
		РегистрыСведений.канбан_ИсторияСтатусовЗадач.ЗакрытьИнтервал(Задача, ОткрытаяЗапись.ИдентификаторЗаписи, Момент);
		
	КонецЕсли;
	
	РегистрыСведений.канбан_ИсторияСтатусовЗадач.ОткрытьИнтервал(Задача, НовыйСтатус, Исполнитель, Момент);
	
КонецПроцедуры

Функция ИспользуетсяЗаписьВРегистр() Экспорт
	
	// TODO нужно в глобальные настройки вынести опцию
	
	Результат = Истина;
	
	канбан_КанбанДоскаПереопределяемый.ИспользуетсяРегистрИсторияСтатусовЗадач(Результат);
	
	Возврат Результат;
	
КонецФункции

Функция ИспользуетсяЗаписьИсторииОшибокВРегистр()
	
	Возврат Метаданные.РегистрыСведений.Найти("канбан_ИсторияСтатусовЗадач") <> Неопределено
		И Метаданные.Справочники.Найти("канбан_Ошибки") <> Неопределено;
	
КонецФункции

Функция ЭтоОшибкаКанбан(Задача)
	
	Возврат Метаданные.ОпределяемыеТипы.канбан_Ошибки.Тип.СодержитТип(ТипЗнч(Задача));
...
Функция НовыйЗадачиИСтатусы()
	...
	МассивТипов.Добавить(Метаданные.ОпределяемыеТипы.канбан_Задачи.Тип.Типы()[0]);
	МассивТипов.Добавить(Метаданные.ОпределяемыеТипы.канбан_Ошибки.Тип.Типы()[0]);
	...
	ЗадачиИСтатусы.Колонки.Добавить("ПредставлениеПользователя", Новый ОписаниеТипов("Строка"));
	...
	ЗадачиИСтатусы.Колонки.Добавить("СрочностьИсправления", Новый ОписаниеТипов("Строка"));
	ЗадачиИСтатусы.Колонки.Добавить("ЭтоОшибка", Новый ОписаниеТипов("Булево"));
...
#Область Порядок
	...
	ПервыйПакет.Порядок.Добавить(ПервыйПакет.Колонки.Найти("КодПроекта"));
	ПервыйПакет.Порядок.Добавить(ПервыйПакет.Колонки.Найти("КодЗадачи"));
...
Процедура ЗаполнитьВсемиПроектами(Знач ТекстЗапросаДанныеЗадач, Проекты)
	...
	ТаблицаПроектов.Сортировать("НаименованиеПроекта");
...
//
Функция НовыйСтатусПоИдентификаторуИМенеджеруОбъекта(
		ЗадачаСсылка, ИдентификаторСтатуса, ИмяМенеджераОбъектаСтатус, Отказ)
	
	УсловиеПроверки = Не ЗадачаСсылка.Пустая();
	...
	НовыйСтатусЗадачи = СсылкаНаОбъектПоИдентификатору(ИмяМенеджераОбъектаСтатус, ИдентификаторСтатуса);// ОпределяемыйТип.канбан_СтатусыЗадач
	...
	Если ИспользоватьСопоставлениеСтатусов И ЭтоТипСтатусОшибки Тогда
		...
		СопоставленныйСтатус(НовыйСтатусЗадачи, СтатусОшибки, Отказ);
		НовыйСтатусЗадачи = СтатусОшибки;
		
	КонецЕсли;
	
	Возврат НовыйСтатусЗадачи;
	
КонецФункции

// Получить значения перечисления по определяемому типу.
// 
// Параметры:
//  ИмяОпределяемогоТипа - Строка
// 
...
//  * name - Строка
//
Функция ПолучитьЗначенияПеречисленияПоОпределяемомуТипу(ИмяОпределяемогоТипа) Экспорт
	...
	Если Метаданные.ОпределяемыеТипы.Найти(ИмяОпределяемогоТипа) = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	ТекущийТип = Метаданные.ОпределяемыеТипы[ИмяОпределяемогоТипа].Тип.Типы()[0];
	Если СтрНайти(ПолноеИмяОбъекта, "Перечисление") > 0 Тогда
		
		ЗначениеПеречисления = Новый(ТекущийТип); // ПеречислениеСсылка
		МенеджерПеречисления = ЗначениеПеречисления.Метаданные();
		
		Для Каждого Перечисление Из МенеджерПеречисления.ЗначенияПеречисления Цикл
			
			СтруктураЗначения = Новый Структура;
			СтруктураЗначения.Вставить("id", Перечисление.Имя);
			СтруктураЗначения.Вставить("name", Перечисление.Синоним);
			
			Результат.Добавить(СтруктураЗначения);
			
		КонецЦикла;
		
	ИначеЕсли СтрНайти(ПолноеИмяОбъекта, "Справочник") > 0 Тогда
		
		МенеджерСправочника = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмяОбъекта);
		Выборка = МенеджерСправочника.Выбрать();
		Пока Выборка.Следующий() Цикл
			
			СтруктураЗначения = Новый Структура;
			СтруктураЗначения.Вставить("id", СтрЗаменить(Строка(Выборка.Ссылка), " ", ""));
			СтруктураЗначения.Вставить("name", Выборка.Наименование);
