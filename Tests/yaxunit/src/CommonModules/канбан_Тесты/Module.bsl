...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты
		...
		.ДобавитьТестовыйНабор("канбан_КанбанДоска")
			...
			.ДобавитьСерверныйТест("ТекстHTMLКанбанДоскиЧетыреПроекта")
			.ДобавитьСерверныйТест("ТекстHTMLКанбанДоскиЧетыреПроектаВключеноСопоставлениеНоНеЗаполнено")
			.ДобавитьСерверныйТест("ТекстHTMLКанбанДоскиЧетыреПроектаССопоставлением")
			.ДобавитьСерверныйТест("ТекстHTMLКанбанДоскиВсеПроекты")
			.ДобавитьСерверныйТест("ТекстHTMLКанбанДоскиЧастичноеОбновление")
			.ДобавитьСерверныйТест("ТекстHTMLКанбанДоскиВсеПроектыССопоставлением")
			.ДобавитьСерверныйТест("ИзменитьСтатусЗадаче")
			.ДобавитьСерверныйТест("ИзменитьСтатусОшибке")
...
//	b49a3ce6-3026-11f0-952c-107b4419808b
	...
	ПервыйПроектНастройки.Цвет = канбан_КанбанДоска.ЦветИзСтроки("238,130,238");
	...
	ВторойПроектНастройки.Цвет = канбан_КанбанДоска.ЦветИзСтроки("204,255,204");
	...
	ТретийПроектНастройки.Цвет = канбан_КанбанДоска.ЦветИзСтроки("251,224,224");
	...
	ЧетвертыйПроектНастройки.Цвет = канбан_КанбанДоска.ЦветИзСтроки("250,211,157");
...
Процедура СопоставленныйСтатус() Экспорт
	...
	СтатусВПроцессеВыполнения = канбан_КанбанДоска.СтатусОбъектаПоИдентификатору(
	...
	ЮТест.ОжидаетЧто(канбан_КанбанДоска)
		.Метод("СопоставленныйСтатус").Параметр(СтатусВПроцессеВыполнения).Параметр(СтатусОшибки)
		.НеВыбрасываетИсключение()
	;
	
	канбан_КанбанДоска.СопоставленныйСтатус(СтатусВПроцессеВыполнения, СтатусОшибки);
	
	ИдентификаторСтатусОшибкиВПроцессеВыполненияСтрока = "d0eb69d9-b7c6-11f0-954d-107b4419808b";
	СтатусОшибкиВПроцессеВыполнения = канбан_КанбанДоска.СтатусОбъектаПоИдентификатору(
...
Процедура ТекстHTMLКанбанДоскиЧетыреПроекта() Экспорт
...
Процедура ТекстHTMLКанбанДоскиЧастичноеОбновление() Экспорт
	
	ТекстДоски = "";
	
	// Получаем исходный текст доски
	ЮТест.ОжидаетЧто(канбан_КанбанДоска)
	...
	РегулярноеВыражение = "v8config:\/\/[a-f0-9]{40}";
	
	ТекстДоски = СтрЗаменитьПоРегулярномуВыражению(ТекстДоски, РегулярноеВыражение, "");
	
	// Имитация нажатия кнопки
	// Из текста доски получаем Документ HTML для того что бы добавить активность кнопке "Только мои задачи"
	ДокументHTML = канбан_КанбанДоска.ПолучитьОбъектДокументHTMLИзТекстаHTML(ТекстДоски);
	УзелТолькоМоиЗадачи = ДокументHTML.ПолучитьЭлементПоИдентификатору("my_tasks");
	АтрибутКлассы = УзелТолькоМоиЗадачи.ПолучитьАтрибут("class");
	АтрибутКлассыНовые = СтрШаблон("%1 %2", АтрибутКлассы, "my_tasks_active");
	УзелТолькоМоиЗадачи.УстановитьАтрибут("class", АтрибутКлассыНовые);
	ТекстДоски = канбан_КанбанДоска.ТекстHTMLИзОбъектаДокументHTML(ДокументHTML);
	
	// Отдаем изменнную доску для обновления отображения задач
	ЮТест.ОжидаетЧто(канбан_КанбанДоска)
		.Метод("ТекстHTMLКанбанДоски").Параметр(ТекстДоски).Параметр("").Параметр(Ложь)
		.НеВыбрасываетИсключение()
	;
	...
	// Получаем эталонный шаблон доски
	МакетТекста = ПолучитьОбщийМакет("канбан_ТекстHTMLДоскиТолькоМоиЗадачи");
	ТекстHTMLДоскиЭталон = МакетТекста.ПолучитьТекст();
	
	ТекстHTMLДоскиЭталон = СтрЗаменитьПоРегулярномуВыражению(ТекстHTMLДоскиЭталон, РегулярноеВыражение, "");
	
	ПолноеИмяОбъектаЗадачи = канбан_КанбанДоска.ПолноеИмяОбъектаМетаданныхОпределяемогоТипа("канбан_Задачи"); // Строка
	ТекстHTMLДоскиЭталон = СтрЗаменить(ТекстHTMLДоскиЭталон, "%1", ПолноеИмяОбъектаЗадачи); // Подставляем полное имя справочника, в зависимости от определяемого типа.
...
Процедура ИзменитьСтатусЗадаче() Экспорт
	...
	ИдентификаторЗадачаСтрока = "b49a3cd7-3026-11f0-952c-107b4419808b";
	...
	ТекущийСтатус = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗадачаСсылка, "Статус");
	...
	ДанныеПриИзмененииСтатуса.ИдентификаторЗадача = ИдентификаторЗадачаСтрока;
...
Процедура СтатусЗадачиПоИдентификатору() Экспорт
	...
	ЮТест.ОжидаетЧто(канбан_КанбанДоска)
		.Метод("СтатусОбъектаПоИдентификатору")
			.Параметр(МетаданныеСсылки.ПолноеИмя()).Параметр(ИдентификаторСсылкиСтрока)
		.НеВыбрасываетИсключение()
		;
	
	ПолученнаяСсылка = канбан_КанбанДоска.СтатусОбъектаПоИдентификатору(
...
Функция СоздатьНовыйОбъектСправочника(ПолноеИмяОбъектаСтатусы, УникальныйИдентификаторСтрока, НаименованиеОбъекта);
	...
	СправочникСсылка = Вычислить(ПолучитьСсылкуСправочника); // СправочникСсылка
	УстановитьБезопасныйРежим(Ложь);
	
	Если ПустаяСтрока(СправочникСсылка.ВерсияДанных) Тогда
...
//
// Параметры:
//   ИмяОпределяемогоТипа - Строка - Имя определяемого типа, для которого необходимо удалить не предопределенные элементы
//
...
Процедура ПроверитьТекстHTMLКанбанДоски(ИмяМакетаЭталона)
